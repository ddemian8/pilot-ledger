...
async function parseReceipt(request: Request, env: Env) {
  ...
  if (env.OPENAI_API_KEY) {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { Authorization: `Bearer ${env.OPENAI_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4.1', input: [{ role: 'system', content: [{ type: 'input_text', text: 'Analizează fotografia ca un specialist în bonuri fiscale din Moldova. Citește fiecare rând vizibil. Pentru amount returnează textul exact al valorii finale de plată de lângă TOTAL, SUMA, TOTAL DE PLATĂ, ACHITAT, ИТОГО sau ОПЛАЧЕНО, inclusiv virgulă și spații; nu folosi subtotalul, TVA-ul, restul sau numărul bonului. Pentru date caută DATA, DATE, ДАТА și convertește în YYYY-MM-DD. currency este MDL dacă apare lei, MDL sau L; EUR doar dacă apare euro/EUR. Returnează exclusiv JSON și null doar dacă valoarea chiar nu este lizibilă. Nu ghici.' }] }, { role: 'user', content }], text: { format: { type: 'json_schema', name: 'receipt', strict: true, schema: { type: 'object', properties: { title: { type: ['string', 'null'] }, category: { type: ['string', 'null'] }, amount: { type: ['string', 'null'] }, currency: { type: ['string', 'null'], enum: ['MDL', 'EUR', null] }, date: { type: ['string', 'null'] } }, required: ['title', 'category', 'amount', 'currency', 'date'], additionalProperties: false } } } }) })
