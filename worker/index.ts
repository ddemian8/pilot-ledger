...
async function parseReceipt(request: Request, env: Env) {
  ...
  if (env.OPENAI_API_KEY) {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { Authorization: `Bearer ${env.OPENAI_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4.1', input: [{ role: 'system', content: [{ type: 'input_text', text: 'Răspunde la întrebarea: „Ce sumă am achitat în acest bon fiscal, cui i-am achitat, când i-am achitat și din ce domeniu este afacerea?”. Analizează fotografia ca un specialist în bonuri fiscale din Moldova și citește fiecare rând vizibil. Include în receipt_text transcrierea exactă a tuturor textelor și numerelor lizibile. Pentru amount returnează textul exact al valorii finale de plată de lângă TOTAL, SUMA, TOTAL DE PLATĂ, ACHITAT, ИТОГО sau ОПЛАЧЕНО, inclusiv virgulă și spații; nu folosi subtotalul, TVA-ul, restul sau numărul bonului. Pentru date caută DATA, DATE, ДАТА și convertește în YYYY-MM-DD. title este comerciantul, business_type este domeniul afacerii, iar category este una dintre Alimentație, Transport, Locuință, Business, Sănătate, Divertisment sau Altele. currency este MDL dacă apare lei, MDL sau L; EUR doar dacă apare euro/EUR. Returnează exclusiv JSON și null doar dacă valoarea chiar nu este lizibilă. Nu ghici.' }] }, { role: 'user', content }], text: { format: { type: 'json_schema', name: 'receipt', strict: true, schema: { type: 'object', properties: { title: { type: ['string', 'null'] }, business_type: { type: ['string', 'null'] }, category: { type: ['string', 'null'] }, amount: { type: ['string', 'null'] }, receipt_text: { type: ['string', 'null'] }, currency: { type: ['string', 'null'], enum: ['MDL', 'EUR', null] }, date: { type: ['string', 'null'] } }, required: ['title', 'business_type', 'category', 'amount', 'receipt_text', 'currency', 'date'], additionalProperties: false } } } }) })
