...
export function ImportSourcePicker({ mode, onChoose }: { mode: 'expense' | 'income'; onChoose: (source: Source, file?: File) => void }) {
  ...
  return <div className="import-picker">
    ...
    <div className="import-options">
      <label className="import-option"><span className="import-option-icon receipt"><Camera size={21} /></span><span><strong>Poză Bon Fiscal</strong><small>Fotografiază sau alege o imagine</small></span><input type="file" accept="image/*" capture="environment" onChange={event => event.target.files?.[0] && onChoose('receipt', event.target.files[0])} /></label>
