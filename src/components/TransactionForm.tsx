...
export function TransactionForm({ mode: startingMode, initial, onSave, onClose, allowType = false }: { mode: 'expense' | 'income'; initial?: Transaction; onSave: (entry: Transaction) => string; onClose: () => void; allowType?: boolean }) {
  const [mode, setMode] = useState(startingMode)
  const [date, setDate] = useState(initial?.date ?? today())
  const [currency, setCurrency] = useState<Currency>(initial?.originalCurrency ?? (initial ? 'EUR' : 'MDL'))
  const [amount, setAmount] = useState(initial ? ((initial.originalCents ?? initial.cents) / 100).toFixed(2) : '')
  ...
  return <form className="entry-form" onSubmit={event => {
    ...
    <label>Descriere<input name="title" defaultValue={initial?.title} autoFocus required maxLength={120} placeholder={mode === 'expense' ? 'Ex. Cumpărături săptămânale' : 'Ex. Salariu'} /></label>
    ...
    <label>Categorie<select key={mode} name="category" defaultValue={initial?.mode === mode ? initial.category : undefined}>{categories.map(category => <option key={category}>{category}</option>)}</select></label>
